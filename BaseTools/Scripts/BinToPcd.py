...
if __name__ == '__main__':
  ...
  if args.PcdName is None:
    ...
    if args.Offset is None:
      ...
      #
      Pcd = '  %s|*|%d|' % (args.PcdName, args.MaxSize)
    else:
      #
      # Use the --offset value provided.
      #
      Pcd = '  %s|%d|%d|' % (args.PcdName, args.Offset, args.MaxSize)
    if args.Verbose:
      print 'PcdToBin: Convert binary file to PCD statement compatible with PCD sections'
    ...
    if args.Offset is None:
      ...
      #
      args.Offset = 0
    Pcd = '  %s|L"%s"|%s|%d|' % (args.PcdName, args.VariableName, args.VariableGuid, args.Offset)
    if args.Verbose:
      print 'PcdToBin: Convert binary file to PCD statement compatible with PCD sections'
  ...
  #
  Pcd = Pcd + '{%s}' % (', '.join(['0x%02x' % (ord(Item)) for Item in Buffer]))
