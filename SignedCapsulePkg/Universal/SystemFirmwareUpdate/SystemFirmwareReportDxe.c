  ...
  )
{
  ...
  *AbortReason     = NULL;
  ...
  DEBUG((EFI_D_INFO, "(Agent)SetImage - LastAttemp Version - 0x%x, State - 0x%x\n", SystemFmpPrivate->LastAttempt.LastAttemptVersion, SystemFmpPrivate->LastAttempt.LastAttemptStatus));
  if (EFI_ERROR(Status)) {
    ...
    DEBUG((EFI_D_INFO, "(Agent)SetLastAttemp - %r\n", VarStatus));
    return Status;
  }
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_INFO, "(Agent)SetImage - SystemFmpProtocol - %r\n", Status));
