...
class CFG_YAML():
    ...
    def __init__(self):
        ...
        self.full_lines = []
        self.index = 0
        self.re_expand = re.compile(r'(.+:\s+|\s*\-\s*)!expand\s+\{\s*(\w+_TMPL) \
                                    \s*:\s*\[(.+)]\s*\}')
...
class CGenYamlCfg:
    ...
    exclude_struct = ['GPIO_GPP_*', 'GPIO_CFG_DATA',
    ...
    def set_field_value(self, top, value_bytes, force=False):
        def _set_field_value(name, cfgs, level):
            ...
            if force or act_cfg['value'] == '':
                value = get_bits_from_bytes(full_bytes, act_cfg['offset'] -
                                            struct_info['offset'],
                                            act_cfg['length'])
                act_val = act_cfg['value']
                if act_val == '':
                    act_val = '%d' % value
                act_val = self.reformat_number_per_type(act_cfg['type'],
                                                        act_val)
